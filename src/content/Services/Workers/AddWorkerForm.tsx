...
const AddLocationForm = dynamic(() => import('./AddLocationForm'), {
  ssr: false
});
...
const WorkerForm = (props: EditFormProps = defaultProps) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const ms = io(
        `${process.env.NEXT_PUBLIC_APP_WS_ENDPOINT}/install_worker`,
        {
          path: '/ws/socket.io/',
          forceNew: true,
      ...
      ms.emit('run_install', {
        ...
        state: formData.provider_location.state,
  ...
  const validationSchema = Yup.object().shape({
    ...
    ipv6: Yup.string().min(1, 'Too Short!').isValidIp6(),
  ...
  useEffect(() => {
    if (!errors.ipv4 && values.ipv4) {
      ...
      (async () => {
        const response = await getWorkers(data);
        if (response.workers.length !== 0) {
          formik.setFieldError('ipv4', 'IP already exists');
        }
      })();
    }
    if (!errors.ipv6 && values.ipv6) {
      const data = {
        search: values.ipv6,
        disabled: '',
        ip_type: '',
      ...
      (async () => {
        const response = await getWorkers(data);
        if (response.workers.length !== 0) {
          formik.setFieldError('ipv6', 'IP already exists');
        }
      })();
    }
  }, [errors]);
